...
function AdoptedPets() {
  ...
  useEffect(()=>{
    let getAllPets = async ()=>{
      let data = await axios.get("http://127.0.0.1:5000/all/adopted/pets")
  ...
  let handleDeleteItems = async(pid,pname,pbreed,pspecies,page,pgender,pimg)=>{
...
<div className='mx-5 d-flex flex-wrap' style={{position:"relative",top:"90px"}}>
<h3 style={{display:"none"}} ref={nopetsAdopted}>NO PETS REQUEST YET</h3>
{
        allPets.map((item,i)=>(
          <div class="card" key={i} style={{maxWidth:"20rem",margin:"20px",minWidth:"15                                                                          rem"}}>
            <img class="card-img-top" src={`http://127.0.0.1:5000/static/images/`+item[7]} alt="Card image cap" style={{maxHeight:"200px"}}/>
            <div class="card-body">
              <h5 class="card-title">Name: {item[1]}</h5>
              <h6>Breed: {item[2]}</h6>
              <h6>Species: {item[3]}</h6>
              <h6 class="card-text">Age:{item[4]}</h6>
              <p class="card-text">Gender:{item[5]}</p>
              {/* <button type='button' data-bs-toggle="modal" data-bs-target="#myModal"  className="btn btn-danger mx-3" onClick={()=>handleDeleteItems(item[0],item[1],item[2],item[3],item[4],item[5],item[7])}>Delete</button> */}
            </div>
          </div>
        ))
      }
